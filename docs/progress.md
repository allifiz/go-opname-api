...
## Current Phase
Core inventory V1, concurrency hardening, authentication/JWT + RBAC, and secure initial-user bootstrap are implemented. The bootstrap batch is awaiting GitHub Actions validation; next focus is menu-template update without mutating historical snapshots.
...
- Secure initial-user provisioning is implemented through `POST /api/v1/auth/bootstrap`:
  ...
  - token is supplied through `X-Bootstrap-Token` and compared using constant-time hashed comparison;
  - name/email/password/role are validated and password is bcrypt-hashed;
  - only seeded business roles are accepted;
  - database transaction locks `users`, verifies the table is empty, and creates exactly one initial user;
  - concurrent bootstrap attempts cannot create multiple initial users;
  - any later bootstrap attempt returns conflict after the first user exists.
- GitHub Actions run #79 passed PostgreSQL startup, migrations, rollback/re-apply, `sqlc generate`, all integration/concurrency/auth tests, build, and generated-code synchronization before the bootstrap batch.
...
## Validation Status
GREEN through GitHub Actions run #79 for the previous auth/RBAC batch. Initial-user bootstrap implementation is awaiting the new run.
...
7. Define the authenticated workflow for provisioning additional users after the one-time bootstrap, without inventing an unapproved operational role.

## Blockers / TBD
- `KEPALA_SPPG` operational permissions remain TBD.
- Authenticated provisioning of additional users is not yet defined because no admin role/permission has been approved in the business flow.
