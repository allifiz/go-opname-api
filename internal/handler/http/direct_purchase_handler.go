...
type DirectPurchaseHandler struct {
	service *service.DirectPurchaseService
}

func NewDirectPurchaseHandler(service *service.DirectPurchaseService) *DirectPurchaseHandler {
	return &DirectPurchaseHandler{service: service}
}

func RegisterDirectPurchaseRoutes(app *fiber.App, handler *DirectPurchaseHandler) {
	api := app.Group("/api/v1")

	api.Post("/purchase-order-items/:id/direct-purchases/shortage", handler.CreateShortage)
	api.Post("/scheduled-menus/:id/direct-purchases/additional-requirement", handler.CreateAdditionalRequirement)
...
func (h *DirectPurchaseHandler) Get(c *fiber.Ctx) error {
...
func (h *DirectPurchaseHandler) ListByScheduledMenu(c *fiber.Ctx) error {
