...
func (s *MaterialUsageService) Create(ctx context.Context, scheduledMenuIDValue string, input CreateMaterialUsageInput) (map[string]any, error) {
    ...
    if len(input.Items) == 0 { return nil, fmt.Errorf("%w: usage items are required", ErrInvalidInput) }

    actualByMaterial := map[string]pgtype.Numeric{}
    ...
    err = s.store.WithTx(ctx, func(q *db.Queries) error {
        ...
        for _, req := range requirements {
            ...
            if _, err := q.CreateMaterialUsageItem(ctx, db.CreateMaterialUsageItemParams{MaterialUsageID: usage.ID, MaterialID: req.MaterialID, PlannedQty: req.PlannedQty, ActualQty: actual, UnitID: req.UnitID}); err != nil { return err }
        }
        return nil
    })
...
func (s *MaterialUsageService) Decide(ctx context.Context, idValue string, input DecideMaterialUsageInput) (map[string]any, error) {
    ...
    err = s.store.WithTx(ctx, func(q *db.Queries) error {
        ...
        for _, item := range items {
            if !numericPositive(item.ActualQty) { if err := q.ConsumeActiveReservationsByScheduledMenuMaterial(ctx, db.ConsumeActiveReservationsByScheduledMenuMaterialParams{ScheduledMenuID: usage.ScheduledMenuID, MaterialID: item.MaterialID}); err != nil { return err }; continue }
